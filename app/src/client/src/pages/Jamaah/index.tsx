...
import { Jamaah, columns } from "./columns";
import { useCRUD } from "@/hooks/backend";
import { AddJamaahForm } from "./add";
...
import { AddJamaahBulk } from "./bulk";

const limit = 20;

export default function JamaahPage() {
  const [page, setPage] = useState(1);
  const [selectedRows, setSelectedRows] = useState<Row<Jamaah>[]>([]);
  const [sorting, setSorting] = useState<SortingState>([]);
  const [searchText, setSearchText] = useState("");
  const debouncedSearchText = useDebounce(searchText, 300);
  const { data, loading, update, remove, create, get } = useCRUD<Jamaah>({
    url: "/jamaah",
    params: {
      page: page.toString(),
  ...
  const tableRef = useRef<TableType<Jamaah>>(null);
  const isFirstRender = useFirstRender();

  function uploadTemplate(file: File) {
    apiFetch({
      url: `${BASE_URL}/jamaah/upload`,
      options: {
        headers: {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4  items-start gap-4 sm:gap-0">
        <div>
          <h1 className="inline-block text-xl font-semibold">Jamaah</h1>
          <p className="text-sm text-muted-foreground">Atur Daftar Jamaah</p>
        </div>
        <div className="flex items-center space-x-4">
          ...
          />
          <AddJamaahForm onSubmit={create}>
            <Button variant="white" className="ml-4">
              <PlusIcon className="mr-2" />
              Add
            </Button>
          </AddJamaahForm>
          <AddJamaahBulk onSubmit={uploadTemplate}>
          ...
          {selectedRows?.length ? (
            <>
              <ConfirmDialog
                title={`Apakah Anda Yakin Untuk Menghapus ${selectedRows.length} Data Jamaah?`}
